...
const ES = process.env.ELASTICSEARCH_URL || process.argv[2] || 'http://localhost:9200';
const U = process.env.ELASTIC_USER || process.argv[3] || 'elastic';
const P = process.env.ELASTIC_PASSWORD || process.argv[4] || 'changeme';
const IDX = 'logs-snmp.topology-default';

function authHeaders() {
  if (process.env.ELASTIC_API_KEY) {
    return { Authorization: `ApiKey ${process.env.ELASTIC_API_KEY}` };
  }
...
// ── Port fault scenarios (admin/oper status across connected switches) ─────
// Link-correlated entries set peer + peerInterface so both ends go oper-down.
const PORT_FAULTS = [
  // HQ-DC1: distribution ↔ access uplink cable failure (both ends)
  {
    device: 'hq-dist-sw-01',
    interface: 'Eth1/1',
    peer: 'hq-access-sw-01',
    peerInterface: 'Eth1/1',
    admin: 'up',
    oper: 'down',
    untilCycle: 2,
  },
  // HQ-DC1: access edge port down (continuity with prior demo)
  {
    device: 'hq-access-sw-03',
    interface: 'Eth1/4',
    peer: null,
    peerInterface: null,
    admin: 'up',
  ...
  // Branch-NYC: distribution ↔ access uplink cable failure (both ends)
  {
    device: 'nyc-sw-01',
    interface: 'Eth1/2',
    peer: 'nyc-sw-03',
  ...
  // Branch-NYC: intentional admin shutdown
  {
    device: 'nyc-sw-02',
    interface: 'Eth1/3',
    peer: null,
  ...
  // Branch-CHI: distribution ↔ access uplink cable failure (both ends)
  {
    device: 'chi-sw-01',
    interface: 'Eth1/2',
    peer: 'chi-sw-02',
...
function buildFaultLookup(faults) {
  const map = new Map();
  for (const f of faults) {
    map.set(`${f.device}|${f.interface}`, {
      admin: f.admin,
      oper: f.oper,
      untilCycle: f.untilCycle,
    });
    if (f.peer && f.peerInterface) {
      map.set(`${f.peer}|${f.peerInterface}`, {
        admin: f.admin,
        oper: f.oper,
        untilCycle: f.untilCycle,
      });
...
const faultLookup = buildFaultLookup(PORT_FAULTS);

function getPortStatus(device, iface, cycle) {
  const f = faultLookup.get(`${device}|${iface}`);
  if (f && cycle < f.untilCycle) {
    return { admin: f.admin, oper: f.oper, down: f.oper === 'down' };
  }
  return { admin: 'up', oper: 'up', down: false };
}

/** True when a link-correlated fault is active between two devices (cycle 0 = current). */
function isLinkFaulted(nameA, nameB, cycle = 0) {
  return PORT_FAULTS.some((f) => {
    if (cycle >= f.untilCycle || f.oper !== 'down' || !f.peer) return false;
    return (
      (f.device === nameA && f.peer === nameB) || (f.device === nameB && f.peer === nameA)
    );
  });
}
...
async function main() {
  ...
  for (let t = 0; t < 5; t++) {
    ...
    for (const d of devs) {
      ...
      for (let i = 0; i < ifs.length; i++) {
        const status = getPortStatus(d.name, ifs[i], t);
        docs.push({
          '@timestamp': ts,
          host: { name: d.name, ip: d.ip, mac: d.mac, type: d.type },
          ...
          interface: {
            ...
            status: { admin: status.admin, oper: status.oper },
            traffic: {
              in: { bytes: Math.floor(Math.random() * 5e8) + 1e6 },
              out: { bytes: Math.floor(Math.random() * 3e8) + 5e5 },
            },
            errors: {
              in: status.down ? Math.floor(Math.random() * 50) : Math.floor(Math.random() * 3),
              out: status.down ? Math.floor(Math.random() * 30) : 0,
            },
          },
        });
      }
  ...
  for (const s of SITES) {
    for (const [si, ti] of s.links) {
      ...
      // Skip MAC learning on links with an active oper-down fault (both ends)
      if (sd.type === 'switch' && !isLinkFaulted(sd.name, td.name, 0))
        mac.push({
          '@timestamp': ts,
          host: { name: sd.name, ip: dIp.get(sd.name), mac: dMac.get(sd.name), type: sd.type },
          observer: { vendor: sd.vendor, sys_descr: sd.descr },
      ...
      if (td.type === 'switch' && !isLinkFaulted(td.name, sd.name, 0))
        mac.push({
          '@timestamp': ts,
          host: { name: td.name, ip: dIp.get(td.name), mac: dMac.get(td.name), type: td.type },
          observer: { vendor: td.vendor, sys_descr: td.descr },
